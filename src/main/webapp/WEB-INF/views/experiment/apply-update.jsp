						...
						<div class="box-body">
							<form id="inputForm" class="form-horizontal" action="${ctx}/apply/${action}" method="post">
								...
								<div class="form-group">
									<label class="col-sm-2 control-label">${field.chViewName}</label>
									<div class="col-sm-10">
										<textarea style="width: 100%;height: 260px;" id="apply_ch_${field.name}" name="ch_${field.name}" <c:if test="${nodeMap[field.id].permissionType eq READ_ONLY}">disabled="disabled"</c:if>>${customField[field.name]['ch'] }</textarea>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-2 control-label">${field.enViewName}</label>
									<div class="col-sm-10">
										<textarea style="width: 100%;height: 260px;" id="apply_en_${field.name}" name="en_${field.name}" <c:if test="${nodeMap[field.id].permissionType eq READ_ONLY}">disabled="disabled"</c:if>>${customField[field.name]['en'] }</textarea>
									</div>
								</div>
								</c:when>
